...
//	销售员userId，事业部商务SYBSWYGBH__c、大区经理DQJLYGBH__c、事业部经理SYBZJLYGBH__c
	 ...
	 * @return
	 */
	public static User queryUserByXSY(Long xsy_c) {
		String sql ="select id,SYBSW__c,SYBSWYGBH__c,DQJL__c,DQJLYGBH__c,SYBZJL__c,SYBZJLYGBH__c from user where id = '"+xsy_c+"'";  
    	try {
    		logger.info("根据授权申请销售员查询用户sql :"+sql);
    		...
    		logger.error("根据授权申请销售员查询用户失败： "+e.getMessage());
    		e.printStackTrace();
    	}
    	return null;
	}
...
//		https://api.xiaoshouyi.com/data/v1/picks/dimension/belongs
        ...
        try {
            ...
            logger.error("查询团队成员报错: "+e.getMessage());
        }catch (IOException e){
            logger.error("查询团队成员报错: "+e.getMessage());
        }
		return null;
