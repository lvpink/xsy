...
public class ProductWs {
    ...
    public static String webservice() {
		...
		List<ZfgCrmMatnrS> items = allItems.subList(0, 100);
		logger.info("WS接口返回产品list大小："+items.size());
		/**构建返回数据*/
		...
		if(null!=items&&items.size()>0){
			...
			for (ZfgCrmMatnrS zfgCrmMatnrS : items) {
				...
				product.setZJBX__c(zfgCrmMatnrS.getZjbx());//整机保修
				product.setHCBX__c(zfgCrmMatnrS.getHcbx());//耗材保修
				...
				product.setSYBJL__c(zfgCrmMatnrS.getSname().equals("") ? null :Long.valueOf(zfgCrmMatnrS.getSname()));//事业部经理
				product.setFHJL__c(zfgCrmMatnrS.getFname().equals("") ? null :Long.valueOf(zfgCrmMatnrS.getFname()));//分货经理
				product.setZCJL__c(zfgCrmMatnrS.getZname().equals("") ? null :Long.valueOf(zfgCrmMatnrS.getZname()));//转储经理
				...
				product.setHTZDS__c(zfgCrmMatnrS.getHtzds().doubleValue());//合同最大数
				product.setBZ__c(zfgCrmMatnrS.getWaers().equals("RMB")? 1:0);//货币码 
				product.setSL__c((double)zfgCrmMatnrS.getTaxrate());//税率
		...
		try {
			...
			List<Product> allproduct = PassUtils.allProduct;
...
//    	webservice();
    	...
    	com.rkhd.platform.sdk.http.Request request = new com.rkhd.platform.sdk.http.Request();
		String m1 ="select id,productName,fscProductModel,fscProductSpec from product"; 
		String[] sql = {m1};
		request.putParameter("sql", sql);
    	QueryAll queryAll = new QueryAll();
    	String execute = queryAll.execute(request, 0l, 0l);
    	logger.info(execute);
